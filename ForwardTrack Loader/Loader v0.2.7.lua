--[[
    Forward Loader v0.2.7 Loader-Only / NextGen
    Aimware v5.1.13 compliant — NO cheat functions, loader-only

    Patches v0.2.7 (build bump v0.2.6 -> v0.2.7):
    - [FIX] Duplicate "Loader Injected" removed (button subtext deleted, only
            progress stage kept) — no 2x data overlap during LOADING/LOADED
    - [FIX] Removed subtitle "loader • secure • verified" under ForwardTrack.fck
    - [FIX] All texts enlarged +1px (title 22, body 16, info 14, mono 11, small 11)
    - [CHG] SYSTEM STATUS rework: ONLINE->OFFLINE (green->red), SECURE tri-state
            INSECURE red / SECURE orange / SECURE green (Secure Boot OFF=red
            INSECURE, ON=orange, verified=green), OUTDATE orange / TO DATE green
    - [BUMP] v0.2.6 -> v0.2.7 version bump, window 375x590
...
local CONFIG = {
    ...
    -- SYSTEM STATUS: ONLINE->OFFLINE, INSECURE/SECURE tri-state, OUTDATE->TO DATE
    features = {
        { label = "ONLINE",   active = true  }, -- 1 ONLINE green / OFFLINE red (static red indicator)
        { label = "INSECURE", active = false }, -- 2 INSECURE red (Secure Boot OFF)
        { label = "SECURE",   active = false }, -- 3 SECURE orange (Secure Boot ON)
        { label = "SECURE",   active = false }, -- 4 SECURE green (Secure Boot ON + verified)
        { label = "TO DATE",  active = true  }, -- 5 TO DATE green / OUTDATE orange
    },
...
-- DRAW HELPERS
------------------------------------------------------------
local function set_color(c,a) draw.Color(c[1],c[2],c[3], a or c[4]) end
...
local function update_loading()
    ...
    if phase_t>=1 then
        ...
        if Loader.phase_index <= #CONFIG.features then
            CONFIG.features[Loader.phase_index].active = true
        end
...
local function update_button(mx, my)
    ...
    -- loader controls toggles (main page 3+)
    local ctrl_w = 90
    local ctrl_h = Layout.options_h
    local ctrl_gap = 10
    -- main page shows first 3 controls only, settings shows all 5
    local show_n = 3
...
local function draw_status_bar()
    ...
    rounded_fill(bar_x,bar_y,bar_x+bar_w,bar_y+bar_h,6,CONFIG.colors.panel)
    rounded_outline(bar_x,bar_y,bar_x+bar_w,bar_y+bar_h,6,CONFIG.colors.border)
    local count=#CONFIG.features; local gap=6; local pad=6
    local avail=bar_w - pad*2 - gap*(count-1)
    local pill_w=l_floor(avail/count)
    fill_rect(bar_x+6,bar_y+1,bar_x+bar_w-6,bar_y+2,CONFIG.colors.hairline,255)
    ...
    -- compute live states per spec
    local is_online = true -- keep ONLINE static, but dot red->green: ONLINE green / OFFLINE red
    -- For demo tie ONLINE to Cloud Sync: Cloud ON = ONLINE, OFF = OFFLINE
    ...
    local online_bd = online_active and CONFIG.colors.accent_dim or col_red

    local secure_on = CONFIG.controls[1].state
    -- tri-state secure: OFF=INSECURE red, ON=SECURE orange (loading) / SECURE green (verified)
    local insecure_active = not secure_on
    local secure_orange_active = secure_on and Loader.state ~= "LOADED"
    local secure_green_active = secure_on and Loader.state == "LOADED"

    local todate_active = CONFIG.controls[2].state -- Cloud Sync ON = TO DATE green else OUTDATE orange
    local todate_label = todate_active and "TO DATE" or "OUTDATE"
    local todate_dot = todate_active and col_green or col_orange
    local todate_bd = todate_active and CONFIG.colors.accent_dim or col_orange

    for idx=1,count do
        local px=bar_x+pad+(idx-1)*(pill_w+gap); local py=bar_y+5; local pw=pill_w; local ph=bar_h-10
        local label, is_active, dot_col, bd_col
        if idx==1 then
            label = online_label; is_active = online_active; dot_col = online_dot; bd_col = online_bd
        elseif idx==2 then
            label = "INSECURE"; is_active = insecure_active; dot_col = col_red; bd_col = col_red
        elseif idx==3 then
            label = "SECURE"; is_active = secure_orange_active; dot_col = col_orange; bd_col = col_orange
        elseif idx==4 then
            label = "SECURE"; is_active = secure_green_active; dot_col = col_green; bd_col = CONFIG.colors.accent_dim
        elseif idx==5 then
            label = todate_label; is_active = true; dot_col = todate_dot; bd_col = todate_bd
        else
            label = CONFIG.features[idx].label; is_active = CONFIG.features[idx].active; dot_col = col_green; bd_col = CONFIG.colors.accent_dim
        end
        -- keep CONFIG.features in sync for click handling (optional)
        CONFIG.features[idx].label = label
        CONFIG.features[idx].active = is_active

        local bg = is_active and CONFIG.colors.panel_light or {20,20,20,255}
        local bd = is_active and bd_col or CONFIG.colors.border
        rounded_fill(px,py,px+pw,py+ph,ph/2,bg)
        rounded_outline(px,py,px+pw,py+ph,ph/2,bd,is_active and 170 or 255)
        local dot_r=3; local dot_cx=px+8; local dot_cy=py+ph*0.5
        set_color(is_active and dot_col or col_gray) draw.FilledCircle(dot_cx,dot_cy,dot_r)
        if is_active then set_color(dot_col,38) draw.FilledCircle(dot_cx,dot_cy,dot_r+2) end
        local txt = truncate_text(Fonts.small, label, pw-18)
        draw.SetFont(Fonts.small) local tw,th=draw.GetTextSize(txt)
        local tx=l_floor(px+15+(pw-15-tw)*0.5); local ty=l_floor(py+(ph-th)*0.5)
        set_color(is_active and CONFIG.colors.text or CONFIG.colors.muted) draw.Text(tx,ty,txt)
    end
...
local function draw_loader_options()
    ...
    local ctrl_y = Layout.options_y
    draw_text_centered(Fonts.small, ui.x + ui.w*0.5, Layout.options_tag_y, "LOADER OPTIONS", CONFIG.colors.muted_dark)
    for i=1, show_n do
        local ctrl = CONFIG.controls[i]
        local cx = x0 + (i-1)*(ctrl_w+gap)
        local hover = (ui.ctrl_hover == i)
        ...
        rounded_fill(cx,ctrl_y,cx+ctrl_w,ctrl_y+ctrl_h, ctrl_h*0.5, bg)
        rounded_outline(cx,ctrl_y,cx+ctrl_w,ctrl_y+ctrl_h, ctrl_h*0.5, bd, hover and 200 or 160)
        local dot_x = cx + 8
        local dot_y = ctrl_y + ctrl_h*0.5
        set_color(ctrl.state and CONFIG.colors.accent or CONFIG.colors.muted_dark)
        draw.FilledCircle(dot_x, dot_y, 3)
        if ctrl.state then set_color(CONFIG.colors.accent, 30) draw.FilledCircle(dot_x, dot_y, 5) end
        ...
        set_color(ctrl.state and CONFIG.colors.text or CONFIG.colors.muted) draw.Text(tx,ty,label)
        local state_txt = ctrl.state and "ON" or "OFF"
        local sc = ctrl.state and CONFIG.colors.accent or CONFIG.colors.muted_dark
...
local function draw_settings()
    ...
    rounded_outline(x, y0+8, x+w, y0+8+Layout.window_end - y0 -16, 6, CONFIG.colors.border, 255)
    draw_text_centered(Fonts.title, ui.x+ui.w*0.5, Layout.settings_title_y, "Loader Settings", CONFIG.colors.text)
    draw_text_centered(Fonts.small, ui.x+ui.w*0.5, Layout.settings_title_y+24, "configure loader behaviour", CONFIG.colors.muted_dark)
    fill_rect(x+20, Layout.settings_title_y+34, x+w-20, Layout.settings_title_y+35, CONFIG.colors.border, 255)
    local row_w = 260
    local row_h = Layout.settings_row_h
    local start_y = Layout.settings_start_y
    local cx = l_floor(ui.x + (ui.w - row_w)*0.5)
    for i, ctrl in ipairs(CONFIG.controls) do
        local ry = start_y + (i-1)*(row_h+8)
        local hover = (ui.settings_hover == i)
        local bg = hover and CONFIG.colors.panel_light or CONFIG.colors.background
        local bd = CONFIG.colors.border
        rounded_fill(cx, ry, cx+row_w, ry+row_h, 6, bg)
        rounded_outline(cx, ry, cx+row_w, ry+row_h, 6, bd, hover and 180 or 255)
        -- label centered left
        draw_text_vcentered(Fonts.info, cx+12, ry, row_h, ctrl.label, CONFIG.colors.text)
        -- toggle switch right
        local tx = cx + row_w - 54
        local ty = ry + 6
        local tw = 42
        local th = 16
        local track_col = ctrl.state and CONFIG.colors.accent_dim or CONFIG.colors.panel
        rounded_fill(tx, ty, tx+tw, ty+th, th/2, track_col)
        rounded_outline(tx, ty, tx+tw, ty+th, th/2, ctrl.state and CONFIG.colors.accent or CONFIG.colors.border, 180)
        local knob_x = ctrl.state and (tx+tw-8) or (tx+8)
        set_color(ctrl.state and CONFIG.colors.accent or CONFIG.colors.muted) draw.FilledCircle(knob_x, ty+th*0.5, 6)
        set_color({245,245,245,255}) draw.FilledCircle(knob_x, ty+th*0.5, 4)
        local st = ctrl.state and "ON" or "OFF"
        draw_text_centered(Fonts.small, tx+tw*0.5, ty+th+5, st, ctrl.state and CONFIG.colors.accent or CONFIG.colors.muted_dark)
    end
...
local function draw_status()
    ...
    if Loader.state=="LOADING" then color=CONFIG.colors.accent elseif Loader.state=="LOADED" then color=CONFIG.colors.accent end
    draw.SetFont(Fonts.info) local txt="Status: "..Loader.state; local tw,th=draw.GetTextSize(txt)
    local dot_x = l_floor(cx - tw*0.5 - 10)
    local dot_y = y + 6
    local pulsing=(Loader.state=="LOADING")
    set_color(color, pulsing and l_floor(70+60*ui.pulse) or 50) draw.FilledCircle(dot_x,dot_y, pulsing and 7 or 5)
    set_color(color,255) draw.FilledCircle(dot_x,dot_y,3)
    draw_text_centered(Fonts.info, cx+4, y, txt, color)
    local hint=Loader.state=="READY" and "Press LOAD to start loader" or (Loader.state=="LOADING" and "Loader working — do not close" or "Loader ready — inject pending")
    draw_text_centered(Fonts.small, cx, y+14, hint, CONFIG.colors.muted_dark)
end
...
callbacks.Register("Draw", "Mictian_Loader_v027_Draw", on_draw)
callbacks.Register("Unload", "Mictian_Loader_v027_Unload", on_unload)
